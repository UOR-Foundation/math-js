 ...
 */
const { PrimeMathError, toBigInt, isPrime, gcd, primeCache, fastExp } = require('./Utils');
...
const _factorizationCache = {
    ...
    prune() {
        ...
        for (const [key, entry] of this.cache.entries()) {
            ...
            const weight = (ageValue * 0.35 + // 35% weight for recency
...
function quadraticSieve(n, options = {}) {
    ...
    for (const dependency of dependencies) {
        ...
        if (dependency.reduce((sum, val) => sum + val, 0) <= 1)
            continue;
...
function factorizeOptimal(n, options = {}) {
    ...
    try {
        ...
        throw new PrimeMathError(`Failed to convert input to BigInt: ${getErrorMessage(error)}`, { cause: { input: String(n).substring(0, 100) + (String(n).length > 100 ? '...' : '') } });
    }
    // Parse options with defaults
    const { advanced = false, useCache = true, parallelizeFactorization = false, partialFactorization = false, validateFactors = true, algorithmParams = {} } = options;
    ...
    if (numDigits <= thresholds.trialDivision) {
        ...
        if (!advanced && !partialFactorization) {
            ...
            throw new PrimeMathError('Number is too large (' + numDigits + ' digits) for non-advanced factorization. ' +
        ...
        if (!partialFactorization) {
            ...
            throw new PrimeMathError('Number is too large (' + numDigits + ' digits) for complete factorization under Prime Framework constraints. ' +
    ...
    if (validateFactors) {
        ...
        if (!isComplete && !partialFactorization) {
            throw new PrimeMathError('Incomplete factorization result. This is a bug in the factorization algorithm.');
        }
...
function factorArrayToMap(factorArray) {
    return new Map(factorArray.map(factor => [toBigInt(factor.prime), toBigInt(factor.exponent)]));
}
/**
