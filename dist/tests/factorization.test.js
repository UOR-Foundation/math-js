const { factorize, factorizeWithPrimes, factorizePollardsRho, factorizeOptimal, factorizeParallel, millerRabinTest, pollardRho, quadraticSieve, ellipticCurveMethod, isFactorizationComplete, fromPrimeFactors, getPrimeFactors, factorMapToArray, factorArrayToMap, getRadical, getPrimeSignature, factorizationCache } = require('../src/Factorization');
...
describe('Factorization Module', () => {
    ...
    describe('Prime Framework specific requirements', () => {
        ...
        test('should properly handle error cases as specified in Prime Framework', () => {
            ...
            // Non-prime factor
            expect(() => fromPrimeFactors(new Map([[4n, 1n]]))).toThrow(PrimeMathError);
            // Zero exponent
            expect(() => fromPrimeFactors(new Map([[2n, 0n]]))).toThrow(PrimeMathError);
            // Negative exponent
            expect(() => fromPrimeFactors(new Map([[2n, -1n]]))).toThrow(PrimeMathError);
        });
