...
function toJSON(data, options = {}) {
    ...
    const { format = 'standard', includeMetadata = false } = options;
    try {
        if (isFactorization) {
...
function convertBaseViaUniversal(value, fromBase, toBase, options = {}) {
    ...
    const { useFactorizationShortcuts = true, useDirectComputation = true } = options;
    // Validate bases
    ...
    if (isPowerOfTwo) {
        ...
        if (factorization.size <= 3) {
            ...
            for (const [prime, exp] of factorization.entries()) {
                if (prime > 1000n)
                    return null; // Too large for direct computation
                value *= prime ** exp;
            }
...
function createConversionPipeline(steps, options = {}) {
    // Default options
    const { parallel = false, batchSize = 100, preserveFactorization = true, streamingOutput = false } = options;
    /**
...
function isPrime(value) {
    ...
    if (value <= 3n)
        return true;
    if (value % 2n === 0n || value % 3n === 0n)
        return false;
    // Use 6k±1 optimization
    let i = 5n;
    while (i * i <= value) {
        if (value % i === 0n || value % (i + 2n) === 0n)
            return false;
        i += 6n;
    }
    return true;
...
function createReferenceFrame(options = {}) {
    const { id = 'canonical', parameters = {} } = options;
...
const Conversion = {
    ...
    batchConvertBase(values, fromBase, toBase, options = {}) {
        const { parallel = false, useFactorization = true } = options;
    ...
    toFactorization(value, options = {}) {
        const { withSignFlag = false, validatePrimality = true, enforceCanonical = true, ...factorizationOptions } = options;
        let isNegative = false;
        let absValue = value;
