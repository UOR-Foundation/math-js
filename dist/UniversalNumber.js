...
class UniversalNumber {
    ...
    toNumber(options = {}) {
        ...
        throw new PrimeMathError(`Value ${this._isNegative ? 'below' : 'above'} ${this._isNegative ? 'MIN' : 'MAX'}_SAFE_INTEGER ` +
    ...
    toApproximateNumber(options = {}) {
        ...
        if (exponent > 1023 || exponent < -1023) {
            if (throwOnOverflow) {
                throw new PrimeMathError(`Exponent ${exponent} is outside the range representable by JavaScript Number ` +
    ...
    formatNumber(options = {}) {
        const { precision = 20, scientific = false, notation = 'standard', base = 10, groupDigits = false, groupSeparator = ',' } = options;
        ...
        if (notation === 'engineering') {
            ...
            if (digitsBeforeDecimal < Math.min(precision, valueStr.length)) {
                mantissa += '.' + valueStr.substring(digitsBeforeDecimal, Math.min(precision + digitsBeforeDecimal, valueStr.length));
            }
            // Format with exponent
            const sign = isNegative ? '-' : '';
        ...
        if (notation === 'compact') {
            ...
            if (digitsBeforeDecimal < Math.min(3, valueStr.length)) {
                mantissa += '.' + valueStr.substring(digitsBeforeDecimal, Math.min(digitsBeforeDecimal + 2, valueStr.length));
            }
            // Format with suffix
            const sign = isNegative ? '-' : '';
    ...
    getNumberParts(options = {}) {
        const { base = 10, includeExponent = true, significantDigits = 15, getSeparateDigits = false } = options;
        // Handle zero case
        if (this._isZero) {
            return {
                isZero: true,
...
UniversalNumber.prototype.modSqrt = function (modulus) {
    ...
    if (a === 0n)
        return new UniversalNumber(0n);
    if (m === 2n)
        return new UniversalNumber(a % 2n);
    ...
    while (t !== 1n) {
        ...
        while (temp !== 1n) {
            ...
            i++;
            if (i >= s)
                return null; // Should not happen if a is a QR
        }
        // Update variables
...
//# sourceMappingURL=UniversalNumber.js.map